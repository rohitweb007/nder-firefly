...
$(document).ready(function() {
  ...
  $('a[href="#beneficiaries"]').on('show', function(e) {
    drawBeneficiary();
  });
...
function updateHeader() {
  var state = categoryControl.getState();
...
function drawCategory() {
  categoryDashboard = new google.visualization.Dashboard(document.getElementById('categoryDashboard'));
  categoryControl = new google.visualization.ControlWrapper({
  ...
  var jsondata = $.ajax({url: "/home/category/chart/" + ID, dataType: "json", async: false}).responseText;
  var data = new google.visualization.DataTable(jsondata);
  google.visualization.events.addListener(categoryControl, 'statechange', updateHeader);
  google.visualization.events.addListener(categoryControl, 'statechange', getSummary);
  google.visualization.events.addListener(categoryControl, 'statechange', drawTransactions);
  google.visualization.events.addListener(categoryControl, 'statechange', drawBudget);
  google.visualization.events.addListener(categoryControl, 'statechange', drawBeneficiary);
  ...
  categoryDashboard.bind(categoryControl, categoryChart);
  categoryDashboard.draw(data);
...
function getSummary(opt) {
  var state = categoryControl.getState();
  $.getJSON('/home/category/summary/' + ID, {start: state.range.start.toDateString(), end: state.range.end.toDateString()}, function(data) {
...
function drawTransactions(opt) {
  ...
  if (workingTransactions === false || (workingTransactions === false && opt && opt.inProgress === false)) {
    ...
    $.getJSON('/home/category/transactions/' + ID, {start: state.range.start.toDateString(), end: state.range.end.toDateString()}, function(data) {
...
function drawBudget(opt) {
  ...
  if (workingBudget === false || (workingBudget === false && opt && opt.inProgress === false)) {
    ...
    $.getJSON('/home/category/budgets/' + ID, {start: state.range.start.toDateString(), end: state.range.end.toDateString()}, function(data) {
...
function drawBeneficiary(opt) {
  ...
  if (workingBeneficiaries === false || (workingBeneficiaries === false && opt && opt.inProgress === false)) {
    ...
    $.getJSON('/home/category/beneficiaries/' + ID, {start: state.range.start.toDateString(), end: state.range.end.toDateString()}, function(data) {
